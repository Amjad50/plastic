...
impl Mapper12 {
    pub fn new() -> Self {
        Self {
            ...
            chr_bank_r4: 0,
            chr_bank_r5: 0,
            chr_low_bit_256: false,
            chr_high_bit_256: false,
    ...
    fn map_ppu(&self, address: u16) -> MappingResult {
        ...
        let extra_256_bit = if address < 0x1000 {
            self.chr_low_bit_256
        } else {
            self.chr_high_bit_256
        } as usize;

        bank |= extra_256_bit << 8;
        ...
        match device {
            Device::CPU => {
                match address {
                    ...
                    0x8000..=0xFFFF => {
                        match address {
                            ...
                            0xC000..=0xDFFF => {
                                if address & 1 == 0 {
                                    // even
                                    // NOTE: this makes this mapper of type MMC3A only, and I
                                    //  hardcoded it because only one game use this mapper, which
                                    //  uses MMC3A, so mostly it will be ok
                                    self.irq_latch = if data < 2 { 2 } else { data };
                                } else {
                                    // odd
                    ...
                    0x4020..=0x5FFF => {
                        self.chr_low_bit_256 = data & 1 != 0;
                        self.chr_high_bit_256 = data & 0x10 != 0;
